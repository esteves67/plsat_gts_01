  ...
  //public  static final String PARM_WORKORDER_ID           = "d_workid";
    ...
    private static void _initDeviceCommandHandlers()
    {
        ...
        String optDevCmdHandlerClasses[] = new String[] {
            ...
            "DeviceCmd_qgv300",
            "DeviceCmd_enfora",
            "DeviceCmd_enfora8750",
            "DeviceCmd_suntech",
            "DeviceCmd_suntechst300ke",
            "DeviceCmd_suntechst330",
            "DeviceCmd_suntechst330v148",
            "DeviceCmd_suntechst600r",
            "DeviceCmd_suntechst910",
            "DeviceCmd_tracer",
            "DeviceCmd_tk104",
	    "DeviceCmd_tk10x",
    ...
    {
//        Print.logWarn("entrando a getDeviceCommandHandler con: " + dcid);
        DCServerConfig dcs = DCServerFactory.getServerConfig(dcid);
        if (dcs != null) {
            // found a DCS configuration
//            Print.logWarn("mostrando DCMap.get(dcid)..........: " + DCMap.get(dcid));
...
//                    Print.logWarn("DCS: " + dcsName);
                    dcHandler = this.getDeviceCommandHandler(dcsName);
//                    Print.logWarn("dcHandler: " + dcHandler);
                    if (dcHandler == null) {
        ...
        if (_uiList) {
            ...
            HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
                public void write(PrintWriter out) throws IOException {
					...
					out.write("<div id='infoDispo'></div>\n");
                    ...
                    out.write("   <th class='"+CommonServlet.CSS_ADMIN_TABLE_HEADER_COL    +"' nowrap>"+FilterText(i18n.getString("DeviceInfo.Statusdev","Status"))+"</th>\n");
                    if (_viewUniqID) {
                    out.write("   <th class='"+CommonServlet.CSS_ADMIN_TABLE_HEADER_COL    +"' nowrap>"+FilterText(i18n.getString("DeviceInfo.vehicleID","Vehicle ID"))+"</th>\n");
                    }                                          
                    ...
                    out.write("   <th class='"+CommonServlet.CSS_ADMIN_TABLE_HEADER_COL    +"' nowrap>"+FilterText(i18n.getString("DeviceInfo.decription","Description"))+"</th>\n");
                    if (_viewSIM) {                
                    ...
                    out.write("   <th class='"+CommonServlet.CSS_ADMIN_TABLE_HEADER_COL    +"' nowrap>"+FilterText(i18n.getString("DeviceInfo.activedev","Active"))+"</th>\n");                    
				   }
                    out.write("  </tr>\n");
                    out.write(" </thead>\n");
                    out.write(" <tbody>\n");
					...
					// Valores del grafico de dispositivos
					int sts1=0;
                    int sts2=0;
                    int sts3=0;
                    int sts4=0;
                    ...
                    for (int d = 0; d < _deviceList.size(); d++) {
                        ...
                        try {
                            ...
                            if (dev != null) {
								...
								// To show last position
                                double  lastLat		= dev.getLastValidLatitude();
                                double	lastLon		= dev.getLastValidLongitude();
                                String linkLocation = "https://maps.google.com/maps?&q="+lastLat+","+lastLon+"("+deviceDesc+")&z=16&t=m&hl=es&output=embed";
								...
								// To check if device is online
                                long todayTime = System.currentTimeMillis()/1000;
                                long lastGPSTime 	= dev.getLastGPSTimestamp();
                                long lastEventTime	= dev.getLastEventTimestamp();
                                String tiempo 		= "";
                                String lastGPS		= reqState.formatDateTime(lastGPSTime);
                                if (StringTools.isBlank(lastGPS)) { 
                                	lastGPS = i18n.getString("DeviceInfo.neverConnected","never");
                                	tiempo=lastGPS;
                                }
                                ...
                                if ( lastLat != 0 && lastLon != 0 && (lastGPSTime == lastEventTime) && (lastGPSTime > 0)){
                                		
                                	totalSegundos = StringTools.parseInt(todayTime - lastEventTime, 0);
                                	
                                	int dias = totalSegundos / 86400;
                                    int horas = (totalSegundos - (dias * 86400))/3600 ; // 24
                                    int minutos = (totalSegundos - (dias * 86400) - (horas*3600))/60;
                                    int segundosR = totalSegundos - (dias * 86400) - (horas*3600) - (minutos*60);
                                    tiempo = ""+ dias + i18n.getString("DeviceInfo.days","d") + "&nbsp;" +
                                    					horas + i18n.getString("DeviceInfo.hours","h") + "&nbsp;" +
                                    					minutos + i18n.getString("DeviceInfo.minutes","m") + "&nbsp;" +
                                    					segundosR + i18n.getString("DeviceInfo.seconds","s");
                                    
                                    if (totalSegundos >=0 && totalSegundos<301){
                                    	statusDev = 1;	// dentro de 5 minutos
                                    	sts1++;
                                    } else if (totalSegundos>=301 && totalSegundos<3601){
                                    	statusDev = 2;	// dentro de la hora
                                    	sts2++;
                                    } else if (totalSegundos>=3601 && totalSegundos<86401){
                                    	statusDev = 3; // mas de una hora pero dentro del dia
                                    	sts3++;
                                    } else if (totalSegundos>86401){
                                    	statusDev = 4; // mas de un dia sin transmitir
                                    	sts4++;
                                    }

                                }else{
                                	totalSegundos = StringTools.parseInt(todayTime - lastGPSTime, 0);
                                    statusDev =5;
                                    sts5++;
                                	// no debe pasar
                                }
                                ...
                                /** PNG ICONS ARRAY
                                 * status_1 : refreshing
                                 * status_2 : online check
                                 * status_3 : alert
                                 * status_4 : error
                                 * status_5 : never 
                                 * **/
                                String statusIcon	= "status_"+ statusDev +".png";
                                ...
                                out.write("   <td class='"+CommonServlet.CSS_ADMIN_TABLE_BODY_COL    +"' "+SORTTABLE_SORTKEY+"='"+ totalSegundos+"' nowrap title="+tiempo+"><img src=./images/"+statusIcon+"></td>\n");
					...
					// Script para grafico de dispositivos
					String sts1_ = i18n.getString("DeviceInfo.STonline","Online");
					String sts2_ = i18n.getString("DeviceInfo.STlessonehours","Less than 1 Hour");
					String sts3_ = i18n.getString("DeviceInfo.STlessoneday","Less than 1 Day");
					String sts4_ = i18n.getString("DeviceInfo.STmoreoneday","More than 1 Day");
					String sts5_ = i18n.getString("DeviceInfo.STnever","Never");
					String sts9_ = i18n.getString("DeviceInfo.STforward","Forward");
					out.println("<script type='text/javascript'>");
                    out.println("$(document).ready(function(){");
                    out.println("  var data = [");
                    out.println("    ['"+sts1_+"', "+sts1+"],['"+sts2_+"', "+sts2+"], ['"+sts3_+"', "+sts3+"],['"+sts4_+"', "+sts4+"],['"+sts5_+"', "+sts5+"], ['"+sts9_+"', "+sts9+"]");
                    out.println("  ];");
                    out.println("  var plot1 = jQuery.jqplot ('infoDispo', [data], {"); 
                    out.println("    title: '"+i18n.getString("DeviceInfo.connectedDevices","Connected Devices")+"', ");
                    out.println("      seriesDefaults: {");
                    out.println("       renderer: jQuery.jqplot.PieRenderer,"); 
                    out.println("        rendererOptions: {");
                    out.println("          showDataLabels: true");
                    out.println("        }");
                    out.println("      },");
                    out.println("      legend: { show:true, location: 'e' }");
                    out.println("    }");
                    out.println("  );");
                    out.println("});");
                    out.println("</script>");
        ...
        if (_uiEdit || _uiView) {
            ...
            HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
                public void write(PrintWriter out) throws IOException {
                    ...
                    out.println(FormRow_TextField(PARM_DEVICE           , true      , i18n.getString("DeviceInfo.deviceID","{0} ID",devTitles)+":"        , _selDevID, 32, 32));
                    ...
                    if (StringTools.isBlank(deviceCode)) {
                        autoFill = i18n.getString("DeviceInfo.serverIdAutoFill","(actualizado automaticamente por el socket DCS)");
                    }
                    if (_viewServID) {
                    ...
                    if (_viewSIM) {
                    out.println(FormRow_TextField(PARM_DEV_SIMPHONE     , _editSIM   , i18n.getString("DeviceInfo.simPhoneNumber2","Numero de Linea#") +":"       , (_selDev!=null)?_selDev.getSimPhoneNumber():"" , 14, 18));
                    }
                    if (_viewSMSEm) {
                    ...
                    */
                    ...
                    out.write(  "</span>\n");
                    out.write("<div style='border: 1px solid black; margin: 2px 20px 5px 10px; height:80px; width:400px; overflow-x: hidden; overflow-y: scroll; background: #fff;'>\n");
                    out.write("<table>\n");
