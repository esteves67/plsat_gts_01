      ...
      //final String  accountID = reqState.getCurrentAccountID();
        ...
        if (rptFormat.equalsIgnoreCase(ReportURL.FORMAT_XLS) ||
            ...
            try {
                ...
                if (count > 0) {			
                    // all is ok
					response.sendRedirect("Track?page=" + rptMenu);       
                    return;
                } else if (count == 0) {				
			        response.sendRedirect("Track?page=" + rptMenu + "&m_contentMessage1");                                       
                    return;
                }
            } 
			catch (Exception e) {
		 ...
		 if (rptFormat.equalsIgnoreCase(ReportData.FORMAT_PDF)) {
            try {
                ...
                if (count > 0) {			
			        ...
			        response.sendRedirect("Track?page=" + rptMenu + "&m_contentMessage2");                                       
                    return;
                }
            } 
			catch (Exception e) {
