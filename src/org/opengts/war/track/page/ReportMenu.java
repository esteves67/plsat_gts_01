    ...
    implements Constants
{
        ...
        throws IOException
    {
      ...
      //out.write("   alert('Report Option: ' + rptOption);\n");
		...
		String pdfMsg   = i18n.getString("ReportMenu.pdfFormat"  ,"Report will be returned as an PDF (Portable Document Format) file");
        ...
        out.write("       } else\n");
		out.write("       if (selVal == '"+ReportURL.FORMAT_PDF+"') {\n");
        out.write("           formatMsgElem.innerHTML = \""+pdfMsg+"\";\n");
        out.write("       } else\n");
        ...
        throws IOException
    {
        ...
        /* has PDF support */
        // You must have the library 'iText-*.*.*.jar' located in the directory CLASSPATH		
        final boolean hasPDFSupport; // see also "ReportURL.FORMAT_PDF"		
        if (!privLabel.getBooleanProperty(PrivateLabel.PROP_ReportMenu_enableReportPDF,false)) {
            hasPDFSupport = false;
        } else {
            hasPDFSupport = true; // EXPERIMENTAL (may not be fully supported)
        }

        /* has XLS/XLSX support */   
        // You must have the library 'poi-*.*-*.jar' located in the directory CLASSPATH		
		// final boolean hasXLSSupport = ReportSpreadsheet.IsExcelSpreadsheetSupported();
        final boolean hasXLSSupport; // see also "ReportURL.FORMAT_XLS" "ReportURL.FORMAT_XLSX"		
        if (!privLabel.getBooleanProperty(PrivateLabel.PROP_ReportMenu_enableReportExcel,false)) {
            hasXLSSupport = false;
        } else {
            hasXLSSupport = true; // EXPERIMENTAL (may not be fully supported)
        }
        ...
        HTMLOutput HTML_CONTENT = new HTMLOutput(CommonServlet.CSS_CONTENT_FRAME, m) {
            public void write(PrintWriter out) throws IOException {
              ...
              //out.write("      <option value='"+ReportURL.FORMAT_TXT  +"'>TXT</option>\n");
				...
				String error_contMsg = i18n.getString("ReportDisplay.pdfNoData","The selected Date/Time range contains no data. (This report has no events).");				
				// Message of error for report Excel				
				String m_contentMessage1 = request.getParameter("m_contentMessage1");	
                // Message of error for report PDF				
				String m_contentMessage2 = request.getParameter("m_contentMessage2");				
                if(m_contentMessage1 != null | m_contentMessage2 != null){
				out.write("<script type='text/javascript'>");				 
                out.write("alert('"+error_contMsg+"')");				               
                out.write("</script>");
				}
