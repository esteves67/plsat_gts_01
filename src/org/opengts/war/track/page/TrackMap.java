    ...
    implements Constants
{
    ...
    private static final boolean SORTABLE_LOCATION_DETAILS      = true;
        ...
        throws IOException
    {
        ...
        if (ListTools.containsIgnoreCase(SHOW_PING_FALSE,showLocateNow)) {
            // explicit "false"
            Print.logInfo("cosa 1..........................");
            commandMap = null;
            deviceSupportsPing = false;
        } else
        if (ListTools.containsIgnoreCase(SHOW_PING_TRUE,showLocateNow)) {
            // explicit "true"
            Print.logInfo("cosa 2..........................");
            Print.logInfo("privLabel..........................:" + privLabel);
            commandMap = device.getSupportedCommands(privLabel,null,"map");
            deviceSupportsPing = true;
        } else {
            // check for other device "ping"
            Print.logInfo("cosa 3..........................");
        ...
        HTMLOutput HTML_CONTENT = new HTMLOutput((mapAutoSize? CommonServlet.CSS_CONTENT_MAP_FULL : CommonServlet.CSS_CONTENT_MAP), m) {
            public void write(PrintWriter out) throws IOException {
                ...
                out.println("\n<!-- Begin Map Controls -->");
                out.println("<td id='"+ID_MAP_CONTROL+"' class='mapControlCell' align='left' valign='top'>"); // style='height:100%;'>");
                ...
                if (mapControlsOnLeft) { // controlsOnLeft/mapOnRight
                    ...
                    */
                    ...
                    if (!isFleet){
						...
						int battLvl = (int)Math.round(device.getLastBatteryLevel() * 100.0) / 100; 						
						out.println("\n<!-- Battery Valor ["+battLvl+"] -->");
                    	...
                    	out.println("<table id='infoDeviceTable' width='275' border='0' cellspacing=''0' cellpadding='0'><tr><td align='center' valign='middle'>");
                    	...
                    	out.println("	<!-- Placa -->");
                    	out.println("	<span class='infoDeviceColumn'>"+i18n.getString("DeviceInfo.licensePlate","License Plate")+":</span>");
                    	out.println("	</td><td>");
                    	out.println("	<span class='infoDeviceTableLicensePlate'>" + placa + "</span>");
                    	out.println("	</td></tr><tr><td>");
                    	out.println("	<!-- Conductor -->");
                    	out.println("	<span class='infoDeviceColumn'>"+i18n.getString("DeviceInfo.driverID","Driver ID")+":</span");
                    	out.println("	</td><td>");
                    	out.println("	<span class='infoDeviceTableValue' title='Valor'>" + device.getDriverID() + "</span>");
                    	out.println("	</td></tr><tr><td>");
                    	/* out.println("	<!-- Simcard -->");
                    	out.println("		<span class='infoDeviceColumn'>"+i18n.getString("DeviceInfo.simPhoneNumber","SIM Phone#")+":</span>");
                    	out.println("	</td><td>");
                    	out.println("	<span class='infoDeviceTableSimcard'>" + simphone + "</span>");
                    	out.println("	</td></tr><tr><td>"); */
                    	out.println("	<!-- ID de Dispositivo -->");
                    	out.println("	<span class='infoDeviceColumn'>ID GPS:</span>");
                    	out.println("	</td><td>");
                    	out.println("	<span class='infoDeviceTableUniqueID'>" + device.getUniqueID() + "</span>");
                    	out.println("	</td></tr><tr><td>");
                    	out.println("	<!-- Odometro -->");
                    	out.println("	<span class='infoDeviceColumn'>"+i18n.getString("DeviceInfo.reportOdometer","Reported Odometer")+":</span>");
                    	out.println("	</td><td>");
                    	int odomDispo = (int)Math.round(device.getLastOdometerKM());
                    	out.println("	<span class='infoDeviceTableOdom'>" + odomDispo + "Kms</span>");
                    	out.println("	</td></tr><tr><td>");
                    	out.println("	<span class='infoDeviceColumn'>"+i18n.getString("DeviceInfo.speedLimit","Maximum Speed")+":</span>");
                    	out.println("	</td><td>");
                    	out.println("	<span class='infoDeviceTableValue' title='Valor'>" + device.getSpeedLimitKPH()+ "</span>");
                    	out.println("	</td></tr></table>");
                    	out.println("</td></tr><tr><td valign='middle' align='center'>");
						
			   // Legend Icons                
			   out.println("<div id='infoLegendDiv'>");
			   if (TrackMap.this.getBooleanProperty(privLabel,PrivateLabel.PROP_TrackMap_showLegend,true)) {
                ...
                out.println("</div>"); // End: Legend Icons
                ...
                out.println("		<td><span class='selectorTitle'>"+i18n.getString("DeviceInfo.select", "Select")+":</span></td>");
                ...
                out.println("<table id='leftMenuTable' border='0' cellpadding='0' cellspacing='0'><tbody>");
                out.println("<tr>");
                out.println("	<td class='leftMenuTableColumn'>");
                ...
                if (TrackMap.this.showFromCalendar) {
                    ...
                    out.println("		<caption class='calendarReportCaption'>"+i18n.getString("TrackMap.selectDates", "Select Date Range:")+"</caption>");
                    out.println("		<tr>");
...
//                        if (!ListTools.isEmpty(commandMap)) {
                            Print.logInfo("primer opcion, al llamar Form_ComboBox........... ");
                            ...
                            out.println("		</form>");
//                        } else {
//                            Print.logInfo("segunda opcion, sin llamar Form_ComboBox........... ");
...
//                    }
                ...
                out.println("<a href='Track?page=map.device' target='_self'><img title='Mapa de Dispositivos' id='botonera' src='images/map_device.png' ></a>");
                out.println("</td>");
                out.println("</tr>");
                
                out.println("<tr>");
                out.println("<td align='center' style='padding:4px 4px 4px 4px;'>");
                out.println("<a href='Track?page=map.fleet' target='_self' style='cursor:pointer;'><img title='Mapa de Grupos' id='botonera' src='images/map_flota.png' ></a>");
                out.println("</td>");
                out.println("</tr>");
                
                out.println("<tr>");
                out.println("<td align='center' style='padding:4px 4px 4px 4px;'>");
                out.println("<img title='Mostrar Calendario' id='calendarReportButton' src='images/show_detail.png'/>");
                out.println("</td>");
                out.println("</tr>");
                
                out.println("<tr>");
                out.println("<td align='center' style='padding:4px 4px 4px 4px;'>");
                out.println("<img title='Mostrar Ruta' id='botonera' src='images/show_route.png' onclick='javascript:mapProviderToggleDetails();' />");
                out.println("</td>");
                out.println("</tr>");
								
				out.println("<tr>");
                out.println("<td align='center' style='padding:4px 4px 4px 4px;'>");
                out.println("<a href='Track?page=menu.rpt.devDetail' target='_self' style='cursor:pointer;'><img title='Reportes' id='botonera' src='images/reports.png' ></a>");
                out.println("</td>");
                out.println("</tr>");
				...
				out.println("<tr>");
                out.println("<td align='center' style='padding:4px 4px 4px 4px;'>");
                out.println("<a href='#' onclick=\"javascript:document.getElementById('LegendPushpins').style.display='block';javascript:document.getElementById('mapProviderLegendT').style.display='block';\" target='_self' style='cursor:pointer;'><img title='Mostrar Leyenda' id='botonera' src='images/pushpins.png' ></a>");
                out.println("</td>");
                out.println("</tr>");
                
                out.println("<tr>");
                out.println("<td align='center' style='padding-left:5px; padding-right:5px;'>");
                out.println("</td>");
                out.println("</tr>");
                out.println("</table>");
            	out.println("</div>");
