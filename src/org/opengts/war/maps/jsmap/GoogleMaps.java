    ...
    extends JSMap
{
    ...
    **/
        ...
        throws IOException
    {
        ...
        JavaScriptTools.writeJSVar(out, "GOOGLE_API_V3"           , this.isVersion3());
        JavaScriptTools.writeJSVar(out, "GOOGLE_MAP_OPTIONS"      , this.getMapOptions(), true);
        JavaScriptTools.writeJSVar(out, "GOOGLE_MAP_INIT_CALLBACK", this.getMapInitCallback(), false);
        JavaScriptTools.writeJSVar(out, "GOOGLE_BINGMAP"          , rtp.getBoolean(PROP_addBingMap,true));
        JavaScriptTools.writeJSVar(out, "GOOGLE_OPENSTREETMAP"    , rtp.getBoolean(PROP_addOpenStreetMap,true));
        // traffic
        JavaScriptTools.writeJSVar(out, "GOOGLE_TRAFFIC_OVERLAY"  , rtp.getBoolean(PROP_addTrafficOverlay,true));
        JavaScriptTools.writeJSVar(out, "TRAFFIC_TEXT"            , trafficText);
        // weather
        JavaScriptTools.writeJSVar(out, "GOOGLE_WEATHER_OVERLAY"  , rtp.getBoolean(PROP_addWeatherOverlay,true));
        ...
        // clouds
        JavaScriptTools.writeJSVar(out, "GOOGLE_CLOUD_OVERLAY"    , rtp.getBoolean(PROP_addCloudOverlay,true));
	...
	Locale locale   = reqState.getLocale();
		...
		// Language support for Google Maps v3
			/* 
		    'ar': 'ARABIC',
            'eu': 'BASQUE',
            'bg': 'BULGARIAN',
        ...
        if (StringTools.isBlank(mapCtlURL)) {
            ...
            if (this.isVersion3()) {
                sb.append("maps.google.com/maps/api/js?v=3&language=" + langCode);
                // weather/cloud
                RTProperties rtp = this.getProperties();
                boolean weather = 
                    rtp.getBoolean(PROP_addWeatherOverlay,true) || 
                    rtp.getBoolean(PROP_addCloudOverlay  ,true);
                if (weather) {
            ...
            if (this.isVersionJS()) {
                sb.append("maps.googleapis.com/maps/api/js?sensor=false&language=" + langCode);
            } else {
                // default to V2
                sb.append("maps.google.com/maps?file=api&v=2");
            }
