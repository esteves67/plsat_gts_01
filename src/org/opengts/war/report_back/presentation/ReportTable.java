    ...
    implements ReportPresentation
{
    ...
    private static final boolean USE_CSV_COLUMN_HEADER_DESCRIPTIONS = true;
    private static final boolean USE_PDF_COLUMN_HEADER_DESCRIPTIONS = true;
        ...
        throws ReportException, IOException
    {
        ...
        switch (rptFormat) {
            ...
            case PDF     :
                return this.writePDF( rd);                    // this.writePDF( out, indentLevel, rd);  
            case HTML    :
            default:
	...
	private int writePDF(ReportData rd) 
        throws ReportException, IOException
    {	
	DBDataIterator data1 = rd.getBodyDataIterator(); 		
	Print.logInfo("data1.hasNext() " + data1.hasNext());
	if (!data1.hasNext()) { // Not exist Data/Events - data1.hasNext() = false
	Print.logInfo("Not exist Data/Events [Report PDF] ");	
	return 0;		
	}	
	/* MIME type */
    // (See "org.opengts.war.track.page.ReportDisplay:writePage")
	...
	document.open();
	document.add(new Paragraph(rd.getReportTitle()));
	document.add(new Paragraph(rd.getReportSubtitle()));
	document.add(new Paragraph(" "));
	// Report Header
	ReportColumn rptCols[]=rd.getReportColumns();
	int len = rptCols.length;
	float[] col = new float[len];
	for (int i = 0; i < len; i++) {
	if(i==4 || i==len-1) {
	col[i]=2;
	}
	...
	boolean useColumnDesc = rd.getProperties().getBoolean(PROP_csvColumnHeaderDescriptions, USE_PDF_COLUMN_HEADER_DESCRIPTIONS);
	...
	if (useColumnDesc) {
	HeaderColumnTemplate hct = this.getHeaderColumnTemplate(dct);
	colTitle = StringTools.replace(hct.getTitle(rd, rptCols[i]),"\n"," ");
	} 
    else {
	colTitle = bct.getFieldName();
	}
   	cell.setBackgroundColor(BaseColor.CYAN);
	cell.setPhrase(new Phrase(colTitle));
	table.addCell(cell);
	}
	}
	/* List of Colors
	...
	*/    
	...
	for(rcdCount = 0; data.hasNext(); rcdCount++) {
	if(rcdCount % 2 == 0){
    ...
    Object fldVal  = dr.getDBValue(fldName, rcdCount, rptCols[i]);
    String valStr  = (fldVal != null)? fldVal.toString() : "";
	cell.setPhrase(new Phrase(valStr));
	table.addCell(cell);
    }					
    }
    }
