...
//    protected static       String HOST_MAPQUEST                 = "localhost:8000";
//    protected static       String HOST_MAPQUEST                 = "104.254.98.118";
    ...
    **/
    private ReverseGeocode getAddressReverseGeocode(GeoPoint gp, String localeStr, boolean cache) 
    {
        ...
        if (xmlDoc == null) {
// felipe
//            String urlWithKeys = this.getAddressReverseGeocodeURLWithKeys(gp);
//            Print.logInfo("Comienza intento con llaves Nominatim: " + urlWithKeys);
//            xmlDoc = GetXMLDocumentWithKeys(urlWithKeys);
//            if (xmlDoc == null) {
//              return null;
//            }
//felipe
...
//            sb.append("http://localhost:8000/nominatim/reverse.php?");
            sb.append("http://18.188.205.95/nominatim/reverse.php?");
            //sb.append("http://104.254.98.110/nominatim/reverse.php?");
//            sb.append("http://104.254.98.118/nominatim/reverse.php?");
...
//        return sbf.toString();
    ...
    private String getAddressReverseGeocodeURLWithKeys(GeoPoint gp)
    {
        StringBuffer sb = new StringBuffer();
        RTProperties rtp = this.getProperties();
        String url = rtp.getString(PROP_reverseURL, null);
...
//            InputStream input = HTMLTools.inputStream_GET(url, 10000);
    ...
    private Document GetXMLDocumentWithKeys(String url)
    {
         try {
            DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
            DocumentBuilder db = dbf.newDocumentBuilder();
...
//            InputStream input = HTMLTools.inputStream_GET(url, 10000);
            ...
            Print.logError("Parse error(with keys): " + pce);
            return null;
        } catch (SAXException se) {
            Print.logError("Parse error(with keys): " + se);
            return null;
        } catch (IOException ioe) {
            Print.logError("IO error(with keys): " + ioe);
            return null;
        }
    }
