...
//    }
    ...
    private byte[] parseInsertRecord_Suntech(String s)
    {   
        // Ascii Data:	
        // Example:
        // -------  
        ...
        Print.logInfo("Parsing(Suntech): " + s);
...
/////        Print.logInfo(fld[3+is_cmd+is_cmd].substring(2,4));
        String fecha = fld[3+is_cmd+is_cmd].substring(6,8) + fld[3+is_cmd+is_cmd].substring(4,6) + fld[3+is_cmd+is_cmd].substring(2,4);
        String hora[] = StringTools.parseString(fld[4+is_cmd+is_cmd], ':');
...
//		boolean validGPS    = fld[3].equalsIgnoreCase("A");	
        double  latitude    = Double.parseDouble(fld[6+is_cmd+is_cmd]);
        double  longitude   = Double.parseDouble(fld[7+is_cmd+is_cmd]);
        double  headingDeg  = Double.parseDouble(fld[9+is_cmd+is_cmd]);
        double  speedKPH    = Double.parseDouble(fld[8+is_cmd+is_cmd]);
		double  hDOP        = 0.0;
		int     numSats     = Integer.parseInt(fld[10+is_cmd+is_cmd]);
                int     gpsFix  = Integer.parseInt(fld[11+is_cmd+is_cmd]);
        double  altitudeM   = 0.0;		
		double  odomM      = Double.parseDouble(fld[12+is_cmd+is_cmd]); // fld[13].substring(4, fld[0].length()); /* Odometer Virtual GPS*/
