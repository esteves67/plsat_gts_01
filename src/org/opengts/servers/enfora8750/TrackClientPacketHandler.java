...
// ----------------------------------------------------------------------------
package org.opengts.servers.enfora8750;
    ...
    extends AbstractClientPacketHandler
{
	...
	if (s.contains("GPRMC")) {    
    return this.parseInsertRecord_Enfora8750(s);
    }
    ...
    private byte[] parseInsertRecord_Enfora8750(String s)
    {   
        // Ascii Data:	
        // Example:
        // -------  
        ...
        Print.logInfo("Parsing(Enfora 8750): " + s);
        ...
        if (fld[0].length() == 21) {
          mobileID = fld[0].substring(2, 11);
          reportType = fld[0].substring(0,1);
        } else {
          mobileID = fld[0].substring(1, 10);
		...
		boolean validGPS    = fld[2].equalsIgnoreCase("A");	
        double  latitude    = validGPS? this._parseLatitude( fld[3], fld[4])  : 0.0;
        double  longitude   = validGPS? this._parseLongitude(fld[5], fld[6])  : 0.0;		
		double  knots       = validGPS? StringTools.parseDouble(fld[7], -1.0) : 0.0;
        double  headingDeg  = validGPS? StringTools.parseDouble(fld[8], -1.0) : 0.0;
        ...
        if (reportType.equals("10")) {
          ...
          statusCode = StatusCodes.STATUS_MOTION_IN_MOTION; //90 degree angle
