...
//    }
    ...
    if (s.contains("+ACK:GTOUT")) {
      return this.parseInsertRecordCommand_Tracer(s);
    }
      return null;
    }
...
/////        Print.logInfo(fld[1]);
        ...
        if (reportType.contains("GTJDR")) {
          myOffset = 3;
        }
        if (reportType.contains("GTIGF") || reportType.contains("GTIGN")) {
          myOffset = 2;
        }
        String eventType = null;
        mobileID = fld[3]; // MDMID      
        eventType = fld[15].trim();
		
		/* parse */
        String fecha = fld[13 - myOffset].substring(6,8) + fld[13 - myOffset].substring(4,6) + fld[13 - myOffset].substring(2,4);
        String hora_formateada = fld[13 - myOffset].substring(8,10) + fld[13 - myOffset].substring(10,12) + fld[13 - myOffset].substring(12,14) + complemento;
...
//		boolean validGPS    = fld[3].equalsIgnoreCase("A");	
        double  latitude    = Double.parseDouble(fld[12 - myOffset]);
        double  longitude   = Double.parseDouble(fld[11 - myOffset]);
        double  headingDeg  = Double.parseDouble(fld[9 - myOffset]);
        double  speedKPH    = Double.parseDouble(fld[8 - myOffset]);
		double  hDOP        = 0;
		double  odomKM      = 0; // fld[13].substring(4, fld[0].length()); /* Odometer Virtual GPS*/

        if (reportType.contains("GTJDR")) {
          hDOP = 0;
          odomKM = 0;
        } else {
          hDOP = Double.parseDouble(fld[7-myOffset]);
          if (reportType.contains("GTDIS") || reportType.contains("GTFRI") || reportType.contains("GTIGL")) {
            odomKM      = Double.parseDouble(fld[19]);
          } else {
            odomKM      = Double.parseDouble(fld[18]);
          }
        }
...
//		int     numSats     = Integer.parseInt(fld[10]);
        ...
        if (reportType.contains("GTIGL")) {
	  statusCode  = StatusCodes.STATUS_LOCATION;
        } else if (reportType.contains("GTIGN")) {
          statusCode  = StatusCodes.STATUS_IGNITION_ON;
        } else if (reportType.contains("GTIGF")) {
          statusCode  = StatusCodes.STATUS_IGNITION_OFF;
        } else if (reportType.contains("GTFRI")) {
            statusCode = StatusCodes.STATUS_LOCATION;
        } else if (reportType.contains("GTRTL")) {
            statusCode = StatusCodes.STATUS_LOCATION;
        } else if (reportType.contains("GTDIS")) {
            if (fld[5].contains("21")) {
              statusCode = StatusCodes.STATUS_PANIC_ON;
            } else if (fld[5].contains("20")) {
...
//            Print.logInfo("Last location >>>> ");
//            Print.logInfo(Double.toString(device.getLastValidLatitude()));
//            Print.logInfo(Double.toString(device.getLastValidLongitude()));
//            Print.logInfo("Last location <<<< ");
        }
    ...
    private byte[] parseInsertRecordCommand_Tracer(String s)
    {   
        if (s == null) {
            Print.logError("String is null");
            return null;
...
//        eventType = fld[15].trim();
        ...
        String fecha = fld[5 - myOffset].substring(6,8) + fld[5 - myOffset].substring(4,6) + fld[5 - myOffset].substring(2,4);
        String hora_formateada = fld[5 - myOffset].substring(8,10) + fld[5 - myOffset].substring(10,12) + fld[5 - myOffset].substring(12,14) + complemento;
        ...
        long    fixtime2  = Long.parseLong(strMiFecha.substring(0,10));
...
//              int     numSats     = Integer.parseInt(fld[10]);
        ...
        if (reportType.contains("GTOUT")) {
          if (commandSerial.contains("FFFE")) {
            statusCode  = StatusCodes.STATUS_SUSPEND;
          }
          if (commandSerial.contains("FFFF")) {
        ...
        Print.logInfo("MobileID  : " + mobileID);
        Print.logInfo("Timestamp: "  + fixtime + " [" + new DateTime(fixtime) + "]");
        Print.logInfo("Timestamp of server: "  + fixtime2 + " [" + new DateTime(fixtime2) + "]");
/////        Print.logInfo("GeoPoint  : " + geoPoint);
        Print.logInfo("Speed    : "  + StringTools.format(speedKPH,"#0.0") + " kph " + headingDeg);
        ...
        GeoPoint geoPoint = new GeoPoint(device.getLastValidLatitude(),device.getLastValidLongitude());
        ...
        this.insertEventRecord(device, 
            fixtime2, statusCode, null/*GeoZone*/,
            geoPoint, gpioInput, speedKPH, headingDeg, altitudeM, odomKM, hDOP);
