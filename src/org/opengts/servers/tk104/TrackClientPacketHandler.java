...
// ----------------------------------------------------------------------------
package org.opengts.servers.tk104;
    ...
    extends AbstractClientPacketHandler
{
	...
	if (s.contains("imei:")) {    
	  return this.parseInsertRecord_Tk104(s);
        }
    ...
    private byte[] parseInsertRecord_Tk104(String s)
    {   
        // Ascii Data:	
        // Example:
        // -------  
        ...
        Print.logInfo("Parsing(Tk104): " + s);

        /* parse to fields */
        String fld_prime[] = StringTools.parseString(s, ':');
        String fld_core = fld_prime[1];
        String fld[] = StringTools.parseString(fld_core, ',');
		...
		/* parse */  		
	long    hms         = StringTools.parseLong(fld[5].substring(0,5), 0L);
        long    dmy         = StringTools.parseLong(fld[2].substring(4,5)+fld[2].substring(2,3)+fld[2].substring(0,1), 0L);
	...
	//double  knots       = validGPS? StringTools.parseDouble(fld[8], -1.0) : 0.0;
        double  headingDeg  = validGPS? StringTools.parseDouble(fld[12], -1.0) : 0.0;
        double  speedKPH    = Double.parseDouble(fld[12]);
	double  hDOP        = 0.0;
	int     numSats     = 0;
	...
	double  odomKM      = 0.0; // fld[13].substring(4, fld[0].length()); /* Odometer Virtual GPS*/
        ...
        if (reportType.equals("move")) {
          statusCode = StatusCodes.STATUS_MOTION_IN_MOTION;
        } else if (reportType.equals("tracker")) {
        ...
        } else if (reportType.equals("help me")) {
          statusCode = StatusCodes.STATUS_PANIC_ON;
        } else if (reportType.equals("73")) {
          statusCode = StatusCodes.STATUS_PANIC_ON;
        } else if (reportType.equals("low battery")) {
