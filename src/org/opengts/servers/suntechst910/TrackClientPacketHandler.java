...
// ----------------------------------------------------------------------------
package org.opengts.servers.suntechst910;
...
//    }
    ...
    if (s.contains("ST910")) {
      if (s.contains("Location")) {
        return this.parseInsertRecord_Suntechst910(s);
    ...
    private byte[] parseInsertRecord_Suntechst910(String s)
    {   
        // Ascii Data:	
        // Example:
        // -------  
        ...
        Print.logInfo("Parsing(Suntechst910): " + s);
...
//        Print.logInfo(fld[1]);
        ...
        String reportType = fld[1]; 
        mobileID = fld[2]; // MDMID      
...
//        Print.logInfo(fld[3+is_cmd+is_cmd].substring(2,4));
        String fecha = fld[4].substring(6,8) + fld[4].substring(4,6) + fld[4].substring(2,4);
        String hora[] = StringTools.parseString(fld[5], ':');
...
//		boolean validGPS    = fld[3].equalsIgnoreCase("A");	
        double  latitude    = Double.parseDouble(fld[7]);
        double  longitude   = Double.parseDouble(fld[8]);
        double  headingDeg  = Double.parseDouble(fld[10]);
        double  speedKPH    = Double.parseDouble(fld[9]);
		double  hDOP        = 0.0;
		int     numSats     = 0;
                int     gpsFix  = Integer.parseInt(fld[11]);
        double  altitudeM   = 0.0;		
		double  odomM      = 0.0; // fld[13].substring(4, fld[0].length()); /* Odometer Virtual GPS*/
        ...
        if (reportType.contains("Location")) {
      	  statusCode = StatusCodes.STATUS_LOCATION;
        } else if (reportType.contains("Emergency")) {
