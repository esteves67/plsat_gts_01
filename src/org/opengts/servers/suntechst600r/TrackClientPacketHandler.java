...
// ----------------------------------------------------------------------------
package org.opengts.servers.suntechst600r;
...
//    }
    ...
    if (s.contains("ST600")) {
      if (s.contains("ALV")) {
        ...
        return this.parseInsertRecord_Suntechst600r(s);
      }
    }
      return null;
    }
    ...
    private byte[] parseInsertRecord_Suntechst600r(String s)
    {   
        // Ascii Data:	
        // Example:
        // -------  
        ...
        Print.logInfo("Parsing(Suntechst600r): " + s);
...
//        Print.logInfo(fld[1]);
        ...
        String cmd_status = null;
        String complemento = ".00";
        int is_cmd = 0;
        int is_typical_cmd = 0;
        if (fld[1].contains("Res")) {
          Print.logInfo("Es comando de respuesta >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
          is_cmd = 1;
          is_typical_cmd = 4;
        }
        String reportType = fld[0]; 
        String eventType = null;
        mobileID = fld[1+is_cmd]; // MDMID      
        if (is_cmd == 0) {
          eventType = fld[20].trim();
        }
		
		/* parse */
//        Print.logInfo(3+is_cmd+is_cmd);
//        Print.logInfo(fld[3+is_cmd+is_cmd]);
//        Print.logInfo(fld[3+is_cmd+is_cmd].substring(6,8));
//        Print.logInfo(fld[3+is_cmd+is_cmd].substring(4,6));
//        Print.logInfo(fld[3+is_cmd+is_cmd].substring(2,4));
        String fecha = fld[4+is_cmd].substring(6,8) + fld[4+is_cmd].substring(4,6) + fld[4+is_cmd].substring(2,4);
        String hora[] = StringTools.parseString(fld[5+is_cmd], ':');
        String hora_formateada = hora[0] + hora[1] + hora[2] + complemento;
        Print.logInfo("Hora formateada..............:" + hora_formateada);
...
//        Print.logInfo("justo despues de asigar fixtime....:");
//        Print.logInfo(Long.toString(fixtime));
//        Date mifecha = new Date();
//        Print.logInfo("justo despues de asigar mifecha....:");
//        Print.logInfo(Long.toString(mifecha.getTime()));
//        String strMiFecha = Long.toString(mifecha.getTime());
//        Print.logInfo(strMiFecha.substring(0,10));
//        long    fixtime     = mifecha.getTime();
//        long    fixtime     = Long.parseLong(strMiFecha.substring(0,10));
		boolean validGPS    = true;
//		boolean validGPS    = fld[3].equalsIgnoreCase("A");	
        double  latitude    = Double.parseDouble(fld[11+is_cmd-is_typical_cmd]);
        double  longitude   = Double.parseDouble(fld[12+is_cmd-is_typical_cmd]);
        double  headingDeg  = Double.parseDouble(fld[14+is_cmd-is_typical_cmd]);
        double  speedKPH    = Double.parseDouble(fld[13+is_cmd-is_typical_cmd]);
		double  hDOP        = 0.0;
		int     numSats     = Integer.parseInt(fld[15+is_cmd-is_typical_cmd]);
                int     gpsFix  = Integer.parseInt(fld[16+is_cmd-is_typical_cmd]);
        double  altitudeM   = 0.0;		
		double  odomM      = Double.parseDouble(fld[17+is_cmd-is_typical_cmd]); // fld[13].substring(4, fld[0].length()); /* Odometer Virtual GPS*/
		double  odomKM     = odomM / 1000;
        long    gpioInput   = 0L; 
        ...
        if (is_cmd == 1) {
          cmd_status = fld[4];
          if (cmd_status.contains("Enable1")) {
            statusCode = StatusCodes.STATUS_RESUME;
          } else if (cmd_status.contains("Disable1")) {
          ...
          if (reportType.contains("STT")) {
            ...
            if (eventType.equals("1")) {
              ...
              Print.logInfo(">>>>>>>>>>>>>>>>>>>>>>> STT con 2");
              Print.logInfo(Double.toString(speedKPH));
              if (speedKPH < 1) {
                statusCode  = StatusCodes.STATUS_LOCATION;
              } else {
          ...
          } else if (reportType.contains("EVT") && eventType.equals("1")) {
              statusCode  = StatusCodes.STATUS_DOOR_OPEN_0;
          } else if (reportType.contains("EVT") && eventType.equals("2")) {
              statusCode  = StatusCodes.STATUS_DOOR_CLOSE_0;

          } else if (reportType.contains("EVT") && eventType.equals("3") && mobileID.equals("205900809")) {
              statusCode  = StatusCodes.STATUS_PANIC_ON;
          } else if (reportType.contains("EVT") && eventType.equals("4") && mobileID.equals("205900809")) {
              statusCode  = StatusCodes.STATUS_PANIC_ON;

          } else if (reportType.contains("EVT") && eventType.equals("3")) {
              statusCode  = StatusCodes.STATUS_DOOR_OPEN_1;
          } else if (reportType.contains("EVT") && eventType.equals("4")) {
              statusCode  = StatusCodes.STATUS_DOOR_CLOSE_1;
...
//        Print.logInfo("mifecha: "  + mifecha + " [" + new DateTime(mifecha) + "]");		
/////        Print.logInfo("GeoPoint  : " + geoPoint);
        ...
        Print.logInfo("GPS FIX: " + gpsFix);
        if (gpsFix == 0) {
            Print.logError("Cadena invalida");
            return null;
        }
        ...
        Print.logInfo("antes ... OdometerKM: " + odomKM);
/*        if (odomKM <= 0.0) {
            Print.logInfo("if ... ");
            // calculate odometer
