...
google.maps.Map.prototype.initGTS = function()
{
    ...
    if (GOOGLE_TRAFFIC_OVERLAY) {
        ...
        traffic.innerHTML = 	TRAFFIC_TEXT;
        
        traffic.index = 1;
        this.controls[google.maps.ControlPosition.BOTTOM_RIGHT].push(traffic);
    ...
    if (GOOGLE_WEATHER_OVERLAY) {
        ...
        weather.innerHTML = 	WEATHER_TEXT;//WEATHER_TEXT; // "Weather";

        weather.index = 5;
        this.controls[google.maps.ControlPosition.BOTTOM_RIGHT].push(weather);
    ...
    if (GOOGLE_CLOUD_OVERLAY) {
        ...
        clouds.innerHTML = 	CLOUD_TEXT;//CLOUD_TEXT; // "Clouds";
        
        clouds.index = 6;
        this.controls[google.maps.ControlPosition.BOTTOM_RIGHT].push(clouds);
        google.maps.event.addDomListener(clouds, 'click', function() {
            if (self.isCloudVisible()) {
                self.hideCloud();
                clouds.className = 'GButton';
                clouds.innerHTML = CLOUD_TEXT;
            } else {
                self.showCloud();
    ...
    if (!GOOGLE_MAP_OPTIONS || (GOOGLE_MAP_OPTIONS == "")) {
        GOOGLE_MAP_OPTIONS = {
            mapTypeControlOptions: {
                ...
                mapTypeIds: [ google.maps.MapTypeId.ROADMAP, google.maps.MapTypeId.HYBRID, google.maps.MapTypeId.TERRAIN, google.maps.MapTypeId.SATELLITE  ] //, fixed 2.4.1-B30
            },
            draggable: true,
    ...
    if (GOOGLE_BINGMAP) {
        ...
        this.gmapGoogleMap.mapTypes.set('bing', new google.maps.ImageMapType({
            ...
            getTileUrl: function(C, Z) { 
                ...
                URL += (Z >= 19)? ".png?g=41&productSet=mmOS" : "?g=550&shading=hill";
                return URL;
            },
            isPng: true,
